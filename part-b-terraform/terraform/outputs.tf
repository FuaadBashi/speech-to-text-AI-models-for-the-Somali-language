...
output "load_balancer_ip" {
  ...
  value       = huaweicloud_vpc_eip.lb.address
}

output "load_balancer_url" {
  description = "Load Balancer URL"
  value       = "http://${huaweicloud_vpc_eip.lb.address}"
}
...
output "autoscaling_group_name" {
  ...
  value       = huaweicloud_as_group.main.scaling_group_name
}

# Security Group Outputs
output "lb_security_group_id" {
  description = "Load Balancer Security Group ID"
  value       = huaweicloud_networking_secgroup.lb.id
}
...
output "quick_start_instructions" {
  ...
  1. Access the application:
     ${huaweicloud_vpc_eip.lb.address}
  
  2. Connect to VPN server:
     ssh -i ${var.key_pair_name}.pem ubuntu@${huaweicloud_vpc_eip.vpn.address}
  
  3. Test instances (if enabled):
     ${join("\n     ", [for ip in huaweicloud_vpc_eip.test_instance[*].address : "ssh -i ${var.key_pair_name}.pem ubuntu@${ip}"])}
