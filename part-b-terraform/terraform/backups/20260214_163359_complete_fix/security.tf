...
resource "huaweicloud_networking_secgroup" "lb" {
...
resource "huaweicloud_networking_secgroup_rule" "lb_http_ingress" {
  security_group_id = huaweicloud_networking_secgroup.lb.id
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 80
...
resource "huaweicloud_networking_secgroup_rule" "lb_https_ingress" {
  security_group_id = huaweicloud_networking_secgroup.lb.id
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
...
resource "huaweicloud_networking_secgroup_rule" "lb_egress" {
  security_group_id = huaweicloud_networking_secgroup.lb.id
  direction         = "egress"
  ethertype         = "IPv4"
  remote_ip_prefix  = "0.0.0.0/0"
...
resource "huaweicloud_networking_secgroup_rule" "web_http_from_lb" {
  ...
  port_range_min    = 80
  port_range_max    = 80
  remote_group_id   = huaweicloud_networking_secgroup.lb.id
  description       = "Allow HTTP from Load Balancer"
}

# Web - Allow SSH from admin (change admin_cidr to restrict)
resource "huaweicloud_networking_secgroup_rule" "web_ssh_ingress" {
  security_group_id = huaweicloud_networking_secgroup.web.id
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
...
resource "huaweicloud_networking_secgroup_rule" "db_mysql_from_web" {
