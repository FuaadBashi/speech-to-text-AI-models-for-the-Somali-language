...
resource "hcs_networking_secgroup" "vpn" {
...
resource "hcs_networking_secgroup_rule" "vpn_ssh_ingress" {
  security_group_id = hcs_networking_secgroup.vpn.id
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 22
...
resource "hcs_networking_secgroup_rule" "vpn_openvpn_ingress" {
  security_group_id = hcs_networking_secgroup.vpn.id
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "udp"
...
# Allow all outbound
resource "hcs_networking_secgroup_rule" "vpn_egress" {
  security_group_id = hcs_networking_secgroup.vpn.id
  direction         = "egress"
  ethertype         = "IPv4"
  remote_ip_prefix  = "0.0.0.0/0"
...
resource "hcs_compute_instance" "vpn" {
  ...
  security_group_ids = [hcs_networking_secgroup.vpn.id]
...
resource "hcs_compute_eip_associate" "vpn" {
  public_ip   = hcs_vpc_eip.vpn.address
  instance_id = hcs_compute_instance.vpn.id
}
